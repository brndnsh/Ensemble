...
export function playHarmonyNote(freq, time, duration, vol = 0.4, style = 'stabs', midi = null, slideInterval = 0, slideDuration = 0, vibrato = { rate: 0, depth: 0 }) {
    ...
    // --- Branch to High Fidelity Engine ---
    if (playback.highFidelity) {
        playHiFiHarmonyNote(freq, playTime, duration, vol, style, midi, slideInterval, slideDuration, vibrato, harmony.activeVoices);
        return;
    }
...
/**
 * HIGH FIDELITY Harmony Engine
 * Features: Unison layering (3x osc), Analog Drift, Stereo Spread
 */
function playHiFiHarmonyNote(freq, time, duration, vol, style, midi, slideInterval, slideDuration, vibrato, voiceList) {
    ...
    if (playback.harmoniesGain) masterGain.connect(playback.harmoniesGain);

    // Add to active voices list for stealing/cleanup
    voiceList.push({ gain: masterGain, time, duration, midi });

    // 2. Analog Drift LFO (Subtle pitch instability)
    const driftLfo = ctx.createOscillator();
    driftLfo.frequency.value = 0.1 + Math.random() * 0.2; // Very slow
    const driftGain = ctx.createGain();
    driftGain.gain.value = 3; // +/- 3 cents approx
    driftLfo.connect(driftGain);
    driftLfo.start(time);

    // 3. Oscillator Stack (Unison)
    // We create 3 layers: Center, Left (Detuned -), Right (Detuned +)
    const layers = [
        { pan: 0, detune: 0, type: 'sawtooth' },
        { pan: -0.6, detune: -12, type: 'sawtooth' }, // Left
        { pan: 0.6, detune: 12, type: 'sawtooth' }    // Right
    ];

    if (style === 'strings' || style === 'smart') {
        // lush super-saw settings
        layers[1].detune = -18;
        layers[2].detune = 18;
    } else if (style === 'organ') {
        // Organs don't detune this way, they use drawbars.
        // For HiFi organ, we use sine layers at harmonic intervals
        layers[0] = { pan: 0, detune: 0, type: 'sine', mult: 1 };
        layers[1] = { pan: -0.3, detune: 2, type: 'sine', mult: 2 }; // Octave
        layers[2] = { pan: 0.3, detune: -2, type: 'sine', mult: 1.5 }; // Fifth
    }

    const activeNodes = [masterGain, driftLfo, driftGain];

    layers.forEach(layer => {
        const osc = ctx.createOscillator();
        osc.type = layer.type;
        const baseFreq = freq * (layer.mult || 1);
        ...
        // Slide Logic
        if (slideInterval !== 0 && slideDuration > 0) {
            const startFreq = baseFreq * Math.pow(2, slideInterval / 12);
            osc.frequency.setValueAtTime(startFreq, time);
            osc.frequency.exponentialRampToValueAtTime(baseFreq, time + slideDuration);
        }

        // Per-layer Filter (for rich movement)
        const filter = ctx.createBiquadFilter();
        filter.type = 'lowpass';

        // Brighter for horns/stabs, darker for strings
        const brightness = (style === 'stabs' || style === 'horns') ? 6 : 3;
        const cutoff = clampFreq(freq * brightness * (1 + playback.bandIntensity));
        filter.frequency.setValueAtTime(cutoff, time);
        // Envelope movement
        filter.frequency.exponentialRampToValueAtTime(clampFreq(freq * 2), time + duration); // Filter closes slightly
        ...
        layerGain.gain.value = 0.33; // Normalize for 3 layers
        ...
        activeNodes.push(osc, filter, panner, layerGain);
    });

    // 4. Master Amplitude Envelope
    const attack = (style === 'stabs' || style === 'plucks') ? 0.01 : 0.3;
    const release = (style === 'stabs') ? 0.1 : 0.5;

    masterGain.gain.setValueAtTime(0, time);
    masterGain.gain.linearRampToValueAtTime(vol * 0.8, time + attack); // 0.8 to compensate for summing
    masterGain.gain.setTargetAtTime(0, time + duration - release, release);

    // Cleanup
    const stopTime = time + duration + 1.0;
    driftLfo.stop(stopTime);

    // Use the first osc's onended as a trigger for cleanup
    activeNodes.find(n => n instanceof OscillatorNode).onended = () => {
        safeDisconnect(activeNodes);
    };
}
